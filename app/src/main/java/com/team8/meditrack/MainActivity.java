...
public class MainActivity extends AppCompatActivity implements OnMapReadyCallback, MedicationAdapter.OnMedicationClickListener {
    ...
    // UI elements - Medication status
    private TextView medicationStatusTextView;
    private TextView lastUpdateTextView;

    // Medication monitoring variables
    private Handler medicationCheckHandler = new Handler();
    private String currentMedicationName = null;
    private boolean medicationTaken = false;
    private Handler medicationTimerHandler = new Handler();
    ...
    private void initializeUI() {
        ...
        // Removed locationTextView initialization

        // Initialize medication status views
    ...
    private void startMedicationCheck() {
        medicationCheckHandler.postDelayed(new Runnable() {
            @Override
            public void run() {
                checkMedicationSchedule();
                // Run again in 10 seconds
                medicationCheckHandler.postDelayed(this, 10000);
            }
    ...
    private void checkMedicationSchedule() {
        try {
            ...
            for (Medication medication : medications) {
                // If current time matches medication time
                if (medication.getTime().equals(currentTime)) {
                    Log.d(TAG, "Time to take medication: " + medication.getName());
                    // Send alert
                    sendMedicationAlert(medication.getName());
                }
            }
        } catch (Exception e) {
            Log.e(TAG, "Error checking medication schedule", e);
        }
    }

    private void sendMedicationAlert(String medicationName) {
        if (mqttHandler == null || !mqttHandler.isConnected()) {
            Log.e(TAG, "Cannot send medication alert: MQTT not connected");
            return;
        }
        ...
        try {
            ...
            Log.d(TAG, "Medication alert sent for: " + medicationName);

            // Update UI to show waiting status
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    medicationStatusTextView.setText("Waiting for " + medicationName + " to be taken...");
                    medicationStatusTextView.setTextColor(getResources().getColor(android.R.color.holo_orange_dark));

                    SimpleDateFormat sdf = new SimpleDateFormat("MMM dd, yyyy HH:mm:ss", Locale.getDefault());
                    lastUpdateTextView.setText("Alert sent: " + sdf.format(new Date()));
                }
            });

            // Start 5-second timer to check if medication was taken
            startMedicationTimer(medicationName);
        } catch (Exception e) {
            Log.e(TAG, "Error sending medication alert", e);
        }
    }

    private void startMedicationTimer(String medicationName) {
        // Cancel any existing timer
        medicationTimerHandler.removeCallbacksAndMessages(null);

        // Set the current medication being monitored
        currentMedicationName = medicationName;
        medicationTaken = false;

        // Start 5-second timer
        medicationTimerHandler.postDelayed(new Runnable() {
            @Override
            public void run() {
                // If medication wasn't taken within 5 seconds
                if (!medicationTaken && currentMedicationName != null &&
                        currentMedicationName.equals(medicationName)) {

                    // Update UI to show not taken status
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            medicationStatusTextView.setText(medicationName + " was not taken");
                            medicationStatusTextView.setTextColor(getResources().getColor(android.R.color.holo_red_dark));

                            SimpleDateFormat sdf = new SimpleDateFormat("MMM dd, yyyy HH:mm:ss", Locale.getDefault());
                            lastUpdateTextView.setText("Last updated: " + sdf.format(new Date()));
                        }
                    });

                    // Reset the current medication
    ...
    private void sendEmergencyAlert() {
        ...
        try {
            ...
            // Start the medication timer for emergency medication
            startMedicationTimer("Emergency Medication");
        } catch (Exception e) {
    ...
    private void updateLocation(String payload) {
        try {
            ...
            // Removed the TextView update
            // Only update map if it's ready
            final double finalLatitude = latitude;
            final double finalLongitude = longitude;
            runOnUiThread(() -> {
                // Removed locationTextView update
    ...
    private void handleMedicationConfirmation(String payload) {
        try {
            ...
            if (payload != null && payload.trim().equals("1")) {
                ...
                // Mark medication as taken to cancel the timer
                medicationTaken = true;

                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        String message = currentMedicationName != null ?
                                currentMedicationName + " has been taken" :
            ...
            // Mark medication as taken to cancel the timer if it matches current medication
            if (taken && currentMedicationName != null && currentMedicationName.equals(medicationName)) {
                medicationTaken = true;
            }
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        // Reset current medication if it matches
                        if (currentMedicationName != null && currentMedicationName.equals(medicationName)) {
                            currentMedicationName = null;
                        }
                    } catch (Exception e) {
    ...
    protected void onDestroy() {
        ...
        // Clean up handlers
        medicationCheckHandler.removeCallbacksAndMessages(null);
        medicationTimerHandler.removeCallbacksAndMessages(null);
